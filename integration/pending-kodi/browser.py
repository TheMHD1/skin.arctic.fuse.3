...
class Browser(xbmcgui.WindowXML):
    ...
    def fetch_entries(self):
        ...
        if self.kind=='live':
            if str(self.category).startswith('jf:collection-'):
                ...
                for offset in range(0,1000,250):
                    ...
                    for row in page['Items']:
                        result.append(entry(row['Name'],{'mode':'shared','kind':'live','id':row['Id'],'type':'TvChannel'},metadata={'channelnumber':row.get('ChannelNumber') or row.get('Number')}))
                    if len(result)>=page['TotalRecordCount']:return result
                raise RuntimeError('Curated collection exceeds safe size')
    ...
    def render(self):
        ...
        for e in self.visible_entries:
            display=channel_display_name if e['params'].get('kind')=='live' else clean_label
            item=xbmcgui.ListItem(label=display(e['label']))
