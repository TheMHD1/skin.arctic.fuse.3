...
with tempfile.TemporaryDirectory(prefix='kodi22-catchup-') as temp:
    ...
    run('git','apply','--check',str(here/'patches/jellyfin-tls-secure-default.patch'),cwd=source)
    run('git','apply',str(here/'patches/jellyfin-tls-secure-default.patch'),cwd=source)
    run('git','apply','--check',str(here/'patches/jellyfin-native-originals.patch'),cwd=source)
    run('git','apply',str(here/'patches/jellyfin-native-originals.patch'),cwd=source)
    shutil.copy2(here/'jellyfin_native_originals.py',source/'jellyfin_kodi/helper/native_originals.py')
    run(sys.executable,str(here/'test-native-originals.py'),str(source))
...
print('PASS: clean 2.2 patches, native-original policy, catch-up contract, browser routing and syntax')
